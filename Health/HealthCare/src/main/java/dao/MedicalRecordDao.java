...
public class MedicalRecordDao {
    ...
    public List<MedicalRecord> getMedicalRecordsByDoctorId(int doctorId) throws SQLException {
        List<MedicalRecord> records = new ArrayList<>();
        String sql = "SELECT mr.record_id, p.name as patient_name, mr.diagnosis, mr.treatment, mr.created_at " +
                     "FROM medical_records mr " +
                     "JOIN patients p ON mr.patient_id = p.patient_id " +
                     "WHERE mr.doctor_id = ? " +
                     "ORDER BY mr.created_at DESC";
    ...
    public List<MedicalRecord> getMedicalRecordsByPatientId(int patientId) throws SQLException {
        List<MedicalRecord> records = new ArrayList<>();
        String sql = "SELECT mr.record_id, mr.diagnosis, mr.treatment, mr.created_at, " +
        ...
        try (Connection connection = DBUtil.getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    record.setCreatedAt(resultSet.getTimestamp("created_at"));
                    record.setDoctorName(resultSet.getString("doctor_name"));
                    record.setSpecialization(resultSet.getString("specialization"));
                    records.add(record);
                }
            }
        }
